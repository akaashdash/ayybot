...
class Search:
    ...
    async def wikipedia(self, ctx, *, query: str):
        ...
        embed = discord.Embed(title=page.title, url=page.url,
                              color=0xFFA500, description=summary)
    ...
    async def image(self, ctx, *, query: str):
        ...
        webpage = "https://www.googleapis.com/customsearch/v1?cx=013629950505680552901%3Axac8ijijt08&searchType=image&key=" + \
            token + "&q=" + query.replace(" ", "+")
        ...
        if match:
            ...
            embed = discord.Embed(
                description="No image found.", color=0xFF0000)
            await self.bot.send_message(ctx.message.channel, embed=embed)
    ...
    async def ub(self, ctx, query: str = "random"):
        ...
        if query == "random":
            ...
            webpage = "http://www.urbandictionary.com/define.php?term=" + \
                query.replace(" ", "+")
        r = requests.get(webpage).text
        soup = BeautifulSoup(r)
        title = soup.find_all(
            "div", {"class": "def-header"})[0].find_all("a")[0].text
        meaning = soup.find_all("div", {"class": "meaning"})[
            0].text.replace("&apos;", "'")
        example = soup.find_all("div", {"class": "example"})[0].text
        embed = discord.Embed(title=title, url=webpage,
                              color=0xFFA500, description=meaning)
